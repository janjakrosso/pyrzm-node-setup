...
# variablelar and binary settings
...
Settings
```console
# seeds, peers, ports, pruning and indexer
...
Initializing a node
```console
# enter the service file as a single command
...
# enter snapshot as a single command
...
# If false is written at the bottom of the output, continue, if not, wait a few minutes.
pryzmd status 2>&1 | jq .SyncInfo
```
```console
# Now we set up our validator
